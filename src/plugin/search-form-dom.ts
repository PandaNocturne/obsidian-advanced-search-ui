...
export function removeDetachedSearchUiFromDocument(containerGroups: Map<HTMLElement, SearchGroup[]>): void {
    activeDocument.querySelectorAll('.asui-search-form-container').forEach(container => {
        containerGroups.delete(container as HTMLElement);
        container.remove();
    });
    activeDocument.querySelectorAll('.advanced-search-ui-toggle-wrapper').forEach(btn => btn.remove());
}
